...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!questionData && courseSlug && isStudentLoggedIn() === true) {
      axios
        .post(API_URL + "quizzes/get_questions.php", { course: courseSlug, user_id: StudentData().ID })
        .then((response) => {
  ...
  function submitAnswers() {
    setPageLoading(true);
    axios
      .post(API_URL + "quizzes/get_questions.php", { user_id: StudentData().ID, course: courseSlug, answers: answersData })
      .then((response) => {
        setTimerActive(false);
        setQuestionData(response.data.questionsData);
        setQuizStatus(response.data.quizStatus);
  ...
  function tryAgain(quiz_course_id) {
    setPageLoading(true);
    axios
      .post(API_URL + "quizzes/retry_quiz.php", { course_id: quiz_course_id, user_id: StudentData().ID })
      .then((response) => {
    ...
    <>
      ...
      <div class="flex justify-between my-5 lg:mt-10 lg:mb-0 lg:mx-48">
        ...
        <Link href={"/courses/" + courseSlug}>
          <a class="lg:bg-gray-200 lg:hover:bg-help text-gray-800 font-bold mx-5 my-2 lg:m-1 lg:py-1 lg:px-4 rounded inline-flex items-start">
                ...
                {Math.round((correctAnswers / quizResult.length) * 100 * 10) / 10 >= 101 ? (
                  <>
                    {/* <Link href={API_URL + "certificates/download.php?course_id=" + course + "&user_id=" + StudentData().ID + "&referer=" + window.location.href}>
                      <a class="bg-help hover:bg-blue-400 text-help-lighter font-bold py-2 px-4 rounded inline-flex items-center">
                  ...
                  <>
                    <button onClick={() => tryAgain(course)} class="bg-help hover:bg-blue-400 text-help-lighter font-bold py-2 px-4 rounded inline-flex items-center">
      ...
      {!quizStatus && quizPoints > 0 ? (
        <>
          ...
          {/* <lottie-player ref={ref} src="https://assets4.lottiefiles.com/packages/lf20_obhph3sh.json" background="transparent" speed="1" style={{ position: "fixed", top: 0, left: 0, zIndex: "30", width: "100%", height: "100vh" }} autoplay></lottie-player> */}
