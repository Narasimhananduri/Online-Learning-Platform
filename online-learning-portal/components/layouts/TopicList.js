      ...
      <>
        <div
          ...
          <ul class="divide-y divide-gray-100">
            <li class="hidden lg:block p-3 bg-help  text-center font-bold font-serif text-white">
              ...
              props.topics.map((topic, key) => (
                <div>
                  <Link href={"/courses/" + props.courseData.Slug + "/" + topic.Slug}>
                    <li class={classNames((topic.Slug === topicSlug ? `bg-gray-200 ` : ``) + ` p-3 hover:bg-gray-200 text-left cursor-pointer flex justify-between items-center `)}>
          ...
          <div className="bg-blue-50 mt-3">
            <p className="p-5">To check your knowledge on {props.courseData.Title}: </p>
            <Link key={props.courseData.ID} href={"/quizzes/" + props.courseData.Slug}>
              <a class="mb-5 mx-5 bg-help hover:bg-blue-400 text-white font-bold py-2 px-4 rounded inline-flex items-center">
