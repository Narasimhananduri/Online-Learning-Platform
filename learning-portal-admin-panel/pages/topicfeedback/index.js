...
export default function Assets() {
  ...
  useEffect(() => {
    axios.post(API_URL + "topicfeedback/get_topicfeedback.php",
     {
  ...
  }, [searchAssetKeyword, assetsUpdated, filterState, filterLocation, filterAssetType, filterStatus, pagination]);
            ...
            <main className="py-10">
              <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8">
                <div className="flex flex-col justify-center lg:flex-row lg:justify-between lg:space-x-5">
                  <div>
                    <h1 className="text-2xl font-bold text-gray-900">Topic Feedback</h1>
                  </div>
                          ...
                          <thead className="bg-gray-50">
                            <tr>
                              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <span class="md:table-cell hidden">Topic Name</span>{" "}
                              </th>
                              ...
                              users1.map((person) => (
                                <tr key={person?.ID}>
                                      ...
                                      <div className="ml-4 space-y-2 md:space-y-0">
                                        <div className="flex md:items-center mr-2 md:mr-0 md:justify-between">
                                          <div className="inline-block ml-3 md:ml-0">
                                            <span class="text-sm md:hidden font-medium">Topic Name :</span>
                                            <div className="text-sm  text-gray-900 inline-block ml-1 md:ml-0">{person?.Title}</div>
                                          </div>
                                        </div>

                                        <div className="flex  md:hidden ml-7 flex-1">
                                          <span class="text-sm md:hidden font-medium">Yes :</span>
                                          <div className="text-sm    md:hidden ml-1">{person?.Yes}</div>
                                        </div>
                                        <div className="flex  md:hidden ml-7 flex-1">
                                          <span class="text-sm md:hidden font-medium">No :</span>
                                          <div className="text-sm  text-gray-900 md:hidden ml-1 ">{person?.No}</div>
                                        </div>

                                        <div class="flex  md:hidden ml-7 flex-1">
                                          <span class="text-sm md:hidden font-medium">Feedback List :</span>
                                          <button
                                            type="button"
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                    <div className="text-sm text-gray-900">{person?.Yes}</div>
                                    {/* <div className="text-sm text-gray-500">City: {person?.email}</div> */}
                                  </td>
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">{person?.No}</td>

                                  <td className="px-6 py-4 whitespace-nowrap hidden md:table-cell">
                                    <a onClick={() => handleenroll(person?.TopicId)} className="text-indigo-600 hover:text-indigo-900 cursor-pointer p-2">
                                      <button
                                        type="button"
                                        class="inline-flex justify-center items-center space-x-2 rounded border font-semibold focus:outline-none px-2 py-1 leading-5 text-sm border-indigo-200 bg-indigo-200 text-indigo-700 hover:text-indigo-700 hover:bg-indigo-300 hover:border-indigo-300 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 active:bg-indigo-200 active:border-indigo-200">
                                        View
